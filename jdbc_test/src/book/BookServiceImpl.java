...
public class BookServiceImpl implements BookService {
	...
	public void startProgram() {
		...
		while(true) {
			...
			case 3 : 
				System.out.println("3. 도서명 정보 삭제");
				deleteBook();
				break;
	...
	public void insertBook() {
		...
		System.out.println("총페이지 수를 입력하세요 >>>>>>>>>");
		int page = sc.nextInt();
		...
		if(resultChk>0) {
			...
			System.out.println("도서 등록이 실패했습니다.");
		}
	...
	public void updatebook() {
		...
		System.out.println("도서ID\t도서명\t저자\t출판사\t출판년도");
		...
		int num = sc.nextInt();
		int boookId = (int) bookList.get(num-1).get("book_id");	
		...
		int resultChk = 0;
		resultChk = bookDAO.updateBook(boookId, updateTitle);
		
		if(resultChk>0) {
			System.out.println("도서가 수정되었습니다.");
		} else {
			System.out.println("도서 수정이 실패했습니다.");
		}
